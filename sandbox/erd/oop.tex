...
\begin{tikzpicture}[node distance=6em, every edge/.style={link}]
  ...
  \node[entity] (task) {Task};
    ...
    \node[attribute] (e_name) [above=2em of task] {Completed?} edge (task);
    \node[attribute] (e_name) [right=2em of task] {Canceled?} edge (task);

  \node[relationship] (works) [below of=task, node distance=12em] {WorksOn} edge (task);
    \node[attribute] (p_est1) [above left=2em of works] {First Estimate} edge (works);
    \node[attribute] (p_est2) [left=2em of works] {Second Estimate} edge (works);
    \node[attribute] (p_est3) [below left=2em of works] {Third Estimate} edge (works);
    \node[attribute] (p_planned) [above right=2em of works] {Planned?} edge (works);
    \node[attribute] (p_act)  [right=2em of works] {Actual} edge (works);
    \node[attribute] (p_inter)  [below right=2em of works] {Interuptions} edge (works);
